...
import { followUserMutation, unfollowUserMutation } from "@/graphql/mutations/user.mutation";
import { getUserById } from "@/graphql/query/user.graphql";
        ...
        <div className="m-2  h-full overflow-y-auto scrollbar-width-none">
          <nav className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-slate-500 text-sm">100 posts</h1>
            </div>
          </nav>
          ...
          <div>
            {user?.posts?.map((item) => (
              <FeedCard data={item as Post} key={item?.id} />
            ))}
          </div>
        </div>
      </MainLayout>
